...
def sample_ppo_params(trial: optuna.Trial) -> Dict[str, Any]:
    ...
    # Whether or not to use bundle of commonly used hyperparameters for PyBullet envs
    use_pybullet_params = trial.suggest_categorical("use_pybullet_params", [False, True])
    policy_kwargs = dict(
        log_std_init=-2,
        ortho_init=False,
        activation_fn=nn.ReLU,
        net_arch=[dict(pi=[256, 256], vf=[256, 256])]
    ) if use_pybullet_params else None
