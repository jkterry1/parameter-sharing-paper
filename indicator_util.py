...
class InvertColorIndicator(AgentIndicator):
    ...
    def apply_space(self, obs_space, agent, use_original_obs):
        ...
        low = new_low.min()
        high = new_high.max()
        channel_low = convert_three_dim(np.full(obs_space.low.shape, low), is_image_space_channels_first(new_low))
        channel_high = convert_three_dim(np.full(obs_space.high.shape, high), is_image_space_channels_first(new_high))
        new_low = np.concatenate([new_low, channel_low], axis = 0 if is_image_space_channels_first(new_low) else 2) if use_original_obs else channel_low
        new_high = np.concatenate([new_high, channel_high], axis = 0 if is_image_space_channels_first(new_high) else 2) if use_original_obs else channel_high
...
class BinaryIndicator(AgentIndicator):
    ...
    def apply_space(self, obs_space, agent, use_original_obs):
        ...
        low = new_low.min()
        high = new_high.max()
        channel_shape = new_low.shape[1:] if is_image_space_channels_first(new_low) else new_low.shape[:2]
        channel_low = convert_three_dim(np.full(channel_shape, low), is_image_space_channels_first(new_low))
        channel_high = convert_three_dim(np.full(channel_shape, high), is_image_space_channels_first(new_high))
        new_low = np.concatenate([new_low, channel_low], axis = 0 if is_image_space_channels_first(new_low) else 2) if use_original_obs else channel_low
        new_high = np.concatenate([new_high, channel_high], axis = 0 if is_image_space_channels_first(new_high) else 2)  if use_original_obs else channel_high
        return Box(low=new_low, high=new_high, dtype=new_low.dtype)
